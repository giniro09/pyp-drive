...
const c=vm.createContext({Math,decision:{},activeLeadTracking:null,capturedLeadKeys:new Set(),branchMults:{left:1.4},cruiseDistanceM:100,consecutiveStraights:0,LABEL_JA:{left:'左折'},getLeadContinuationBranchId:()=> 'left',defaultBrId:null,bustProb:.1,LEAD_TYPES:types,discoveryCollection:{sets:{}},generateLeadItem:lead=>({name:'応急シール材',credits:80,effect:'帰還時の売却価値 +80G',type:lead.type})});
...
assert.doesNotMatch(rendered,/280m/);
assert.match(rendered,/>\+140 pt</);
assert.match(rendered,/>\+80G</);
assert.doesNotMatch(rendered,/MOBILE DEPOT/);
...
assert.doesNotMatch(rendered,/295m/);
assert.match(rendered,/その先で装備パーツ ×1/);
assert.match(rendered,/>追跡中</);
assert.doesNotMatch(rendered,/SIGNAL RUNNER/);
...
console.log('PASS: branch choices show committed points and concrete rewards without internal names or arbitrary distance');
